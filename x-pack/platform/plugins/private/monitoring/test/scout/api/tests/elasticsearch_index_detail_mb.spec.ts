...
/**
 * Migrated from:
 *   x-pack/platform/test/functional/apps/monitoring/elasticsearch/index_detail_mb.js
 *   (Active Indices — tests #44)
 *
...
apiTest.describe(
  'Elasticsearch index detail mb — active indices (singlecluster_three_nodes_shard_relocation_mb)',
  ...
  () => {
    ...
    for (const index of INDICES) {
      apiTest(
        `index "${index.name}" (mb) has correct summary (health: ${index.expectedHealth})`,
        async ({ apiClient }) => {
    ...
    apiTest('phone-home index (mb) shows logs link', async ({ apiClient }) => {
      const response = await apiClient.post(
