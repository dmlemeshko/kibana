 ...
 *   (cluster summary assertion — #39)
 *
 * The sorted-rows table test (#40) is deferred (EUI issue #1322).
 * Filter tests (#41) are UI tests → elasticsearch_indices_filtering.spec.ts
 */
...
apiTest.describe(
  'Elasticsearch indices listing — cluster summary (singlecluster_red_platinum)',
  ...
  () => {
    ...
    apiTest('cluster summary has correct info for indices listing', async ({ apiClient }) => {
